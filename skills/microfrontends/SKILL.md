...
- `add-to-group` — add the current project to an existing group; interactive if `--group`/`--default-route` omitted (options: `--group`, `--default-route`); a billing confirmation prompt is still required when adding would exceed the plan's included project limit
