...
**Why the rename in Next.js 16**: `middleware.ts` → `proxy.ts` better reflects its purpose and clarifies it sits at the network boundary (not general-purpose middleware). The exported function must also be renamed from `middleware` to `proxy`. Migration codemod: `npx @next/codemod@latest middleware-to-proxy .`
