...
#### 6d. Drain Signature Secret Set?

Check whether the project has a `DRAIN_SECRET` (or similar) environment variable configured:

```bash
vercel env ls
```

Look for `DRAIN_SECRET` or `VERCEL_DRAIN_SECRET` in the output. If drains are configured but no signature secret is found:

```
⚠️ Drains are configured but no DRAIN_SECRET env var detected.
Without signature verification, drain endpoints are vulnerable to spoofed payloads.
Set DRAIN_SECRET to the signing secret shown when the drain was created.
...
| Situation | Action |
|-----------|--------|
| No drains configured (Hobby plan) | Use Dashboard Logs tab, `vercel logs <url> --follow`, or MCP `get_runtime_logs` |
| No drains configured (Pro/Enterprise) | Recommend configuring drains via Dashboard or REST API (`/v1/drains`) |
| Drain in error state | Test endpoint reachability → verify format → POST `/v1/drains/<id>/test` → recreate if needed |
| Production 500s detected | Correlate: `vercel logs <url> --level error --since 1h` + check drain payloads for matching timestamps + traces |
| Analytics not instrumented | Install `@vercel/analytics` and add `<Analytics />` to root layout |
| Speed Insights not instrumented | Install `@vercel/speed-insights` and add `<SpeedInsights />` to root layout |
| Drain secret missing | Set `DRAIN_SECRET` env var from drain creation secret |
