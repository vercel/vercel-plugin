...
| **Manifest** | The pre-compiled skill index at `generated/skill-rules.json`. Built by `scripts/build-manifest.ts` from `engine/*.md` rule files, it converts glob patterns to regex at build time. Version 2 format with paired arrays (`pathPatterns` ↔ `pathRegexSources`, etc.). |
...
| **Skill** | A unit of injectable knowledge. Matching metadata is defined in `engine/<name>.md` (compiled into `generated/skill-rules.json`). Full skill content is resolved at runtime from the `~/.vercel-plugin/` cache (installed via registry or docs fallback). Skills are the unit of domain knowledge. |
...
| **Engine rule** | A markdown file in `engine/<name>.md` with YAML frontmatter defining skill metadata (patterns, priority, prompt signals, validation rules). Compiled into `generated/skill-rules.json` by `scripts/build-manifest.ts` for fast runtime matching. |
