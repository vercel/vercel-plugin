...
retrieval:
  aliases: ["vercel ai", "ai sdk", "ai library", "ai module"]
  intents: ["add AI-powered text generation or chat to my app", "stream responses from a language model to the UI", "call tools and run agent loops with an LLM", "generate structured JSON output from a prompt", "build ai feature with streaming and tool calling"]
  entities: ["useChat", "useCompletion", "streamText", "generateText", "generateObject", "CoreMessage", "ToolLoopAgent", "AI Gateway"]
chainTo:
  - pattern: "process\\.env\\.(OPENAI_API_KEY|ANTHROPIC_API_KEY)|openai\\([''\"]|anthropic\\([''\"]|\\bgpt-4o\\b"
    targetSkill: ai-gateway
    message: "Direct provider API key or stale model detected — loading AI Gateway guidance for OIDC auth, routing, and failover."
...
Guidance for ai-sdk. Install from registry for full content.
