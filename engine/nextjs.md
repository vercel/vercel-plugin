...
retrieval:
  aliases: ["next.js", "nextjs app", "react framework", "app router"]
  intents: ["set up routing and layouts in a Next.js app", "choose between server and client components for a feature", "configure data fetching or caching in App Router", "add middleware or proxy logic to handle requests", "set up server rendering for React pages", "add a new page with dynamic route segments"]
  entities: ["App Router", "Server Components", "Server Actions", "generateMetadata", "layout", "proxy", "next.config"]
chainTo:
  - pattern: "export\\s+(default\\s+)?function\\s+middleware"
    targetSkill: routing-middleware
    message: "middleware() is renamed to proxy() in Next.js 16 — loading Routing Middleware guidance for proxy.ts migration."
