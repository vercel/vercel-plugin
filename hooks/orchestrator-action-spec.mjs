...
function blockedReasonForAction(plan, actionId) {
  switch (actionId) {
    ...
    case "vercel-env-pull":
      ...
      return null;
    case "vercel-deploy":
      return plan.vercelLinked ? null : "Link the project first; `vercel deploy` is only runnable after the project is linked.";
  }
}
function withRunState(plan, spec) {
...
function getOrchestratorActionSpecs(plan) {
  ...
  return [
    ...
    withRunState(plan, {
      ...
      description: installMissing?.description ?? "Install detected skills into the project skill cache.",
