/**
 * playbook-distillation.mts — Distill ordered multi-skill playbooks from
 * grouped SkillExposure records.
 *
 * Groups exposures by `exposureGroupId`, derives ordered unique skill sequences
 * capped by `maxSkills`, and emits `promote` only when support, precision, lift,
 * and stale-miss thresholds all pass. The distiller compares each playbook
 * (ordered sequence) against the anchor skill's baseline performance without
 * that exact sequence.
 */
...
export interface DistillPlaybooksParams {
  projectRoot: string;
...
function orderedUnique(skills: string[]): string[] {
...
export function distillPlaybooks(
  params: DistillPlaybooksParams,
  ...
  // Bucket playbook sequences and anchor baselines
  const playbookBuckets = new Map<string, PlaybookBucket>();
  const anchorBaselines = new Map<string, BaselineBucket>();
  ...
  for (const [groupId, group] of byGroup) {
    ...
    // Update anchor baseline (all groups with this anchor, regardless of sequence)
    ...
    if (
      ...
      candidate.outcome === "directive-win"
    ) {
      baseline.wins += 1;
    }
    ...
    // Only bucket multi-skill sequences
  ...
  // Evaluate each playbook against anchor baseline
  const rules: LearnedPlaybookRule[] = [];
  ...
  for (const bucket of playbookBuckets.values()) {
    ...
    const liftVsAnchorBaseline =
      baselinePrecisionWithoutPlaybook === 0
        ? precisionWithPlaybook
        : precisionWithPlaybook / baselinePrecisionWithoutPlaybook;
    ...
    const promoted =
      ...
      precisionWithPlaybook >= minPrecision &&
      liftVsAnchorBaseline >= minLift &&
    ...
    rules.push({
      ...
      reason: promoted
        ...
        : "insufficient support, precision, lift, or stale-miss performance",
  ...
  rules.sort(
    (a, b) =>
      ...
      a.anchorSkill.localeCompare(b.anchorSkill) ||
      a.orderedSkills.join(">").localeCompare(b.orderedSkills.join(">")),
  );
